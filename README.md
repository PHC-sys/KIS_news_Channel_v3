...
### 기사 묶기 — 동일 주제·동일 기업 복수 기사
같은 이슈 또는 동일 기업에 대한 기사가 여럿일 때, **가장 정보가 풍부한 기사 1건을 대표로 선택**해 그 기사의 제목과 RSS 요약을 그대로 사용하고, 나머지 기사의 URL만 링크로 나란히 첨부한다.

**절대 금지: 여러 기사를 합쳐서 새 요약을 만들어내지 않는다.** 요약은 반드시 대표 기사의 RSS 내용만 근거로 한다.

```
<b>1. 제목...핵심 키워드</b>
<blockquote>대표 기사의 RSS 요약 그대로. 임의 합성 금지.
<a href="URL1">[WSJ]</a> · <a href="URL2">[FT]</a></blockquote>
```

적용 예시:
- 같은 Fed 발언을 WSJ·FT가 각각 보도 → WSJ 기사 대표로, FT 링크 추가
- SpaceX 관련 기사 2건 → 정보 더 많은 쪽 대표로, 나머지 링크 추가
- 동일 관세 이슈 복수 각도 기사 → 대표 1건 요약, 나머지 링크 2개 추가
