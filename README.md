...
같은 이슈 또는 동일 기업에 대한 기사가 여럿일 때, **대표 기사 하나로 요약을 통합**하고 나머지 URL은 링크로 나란히 첨부한다. 별도 항목으로 쪼개지 않는다.

```
<b>1. 제목...핵심 키워드</b>
<blockquote>2~3줄 한국어 요약. 복수 기사를 통합 서술.
<a href="URL1">[WSJ]</a> · <a href="URL2">[FT]</a></blockquote>
```

적용 예시:
- 같은 Fed 발언을 WSJ·FT가 각각 보도 → 하나로 묶어 링크 2개
- SpaceX 관련 기사 2건 → 하나로 묶어 링크 2개
- 동일 관세 이슈 복수 각도 기사 → 하나로 묶어 링크 2~3개
