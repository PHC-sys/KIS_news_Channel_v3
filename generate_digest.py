...
def plan_digest(client: anthropic.Anthropic, articles: list[dict]) -> str:
    ...
    msg = (
        ...
        "   - 테러·군사 충돌 단신 (예: IS 전투원 사살, 드론 격추 등 — 지정학 리스크 확대가 아닌 것)\n"
        "   - 전염병·보건 이슈 (금리·환율·원자재에 직접 영향 없는 것, 예: 에볼라·코로나 단신)\n"
        "   - 지역 선거·내정 (단, 영국·유로존·미국 정치는 환율·금리 영향 있으면 포함)\n"
        "   - 개별 기업 M&A·지분 취득 (시장 전체 영향 없는 것)\n"
        "   - 개별 기업 내부 노사 이슈 (예: 파업 가처분 등)\n"
        "   - 특정 기업의 섹터 저평가 주장·자사 투자 발표 (예: 화학사 자사주 매입)\n"
